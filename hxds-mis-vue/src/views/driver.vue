  ...
  <div v-if="isAuth(['ROOT', 'DRIVER:SELECT'])">
    <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
      <el-form-item prop="name">
        <el-input v-model="dataForm.name" placeholder="姓名" size="medium" class="input"
                  clearable="clearable"/>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item prop="tel">
        <el-input v-model="dataForm.tel" placeholder="电话" size="medium" class="input"
                  clearable="clearable"/>
      </el-form-item>
      <el-form-item prop="pid">
        <el-input v-model="dataForm.pid" placeholder="身份证" size="medium"
                  clearable="clearable"/>
      </el-form-item>
      <el-form-item>
        <el-select v-model="dataForm.realAuth" class="input" placeholder="实名认证" size="medium"
                   clearable="clearable">
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" cell-style="padding: 4px 0" style="width: 100%;"
              size="medium" @expand-change="expand" :row-key="getRowKeys" :expand-row-keys="expands">
          ...
          <div class="content-container">
            <div class="left">
              ...
              <ul class="credentials-list">
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.idcardFront"
                            :preview-src-list="content.idcardFrontList" fit="cover"/>
                  <p>【 身份证正面 】</p>
                </li>
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.idcardBack"
                            :preview-src-list="content.idcardBackList" fit="cover"/>
                  <p>【 身份证背面 】</p>
                </li>
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.idcardHolding"
                            :preview-src-list="content.idcardHoldingList" fit="cover"/>
                  <p>【 手持身份证 】</p>
                </li>
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.drcardFront"
                            :preview-src-list="content.drcardFrontList" fit="cover"/>
                  <p>【 驾驶证正面 】</p>
                </li>
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.drcardBack"
                            :preview-src-list="content.drcardBackList" fit="cover"/>
                  <p>【 驾驶证背面 】</p>
                </li>
                <li class="item">
                  <el-image style="width: 350px; height: 220px" :src="content.drcardHolding"
                            :preview-src-list="content.drcardHoldingList" fit="cover"/>
                  <p>【 手持驾驶证 】</p>
                </li>
              </ul>
            </div>
      ...
      <el-table-column header-align="center" align="center" width="250" label="操作">
        <template #default="scope">
          <el-button type="text" size="medium" v-if="isAuth(['ROOT', 'USER:UPDATE'])"
                     @click="showApproveModel(scope.row.id)" :disabled="scope.row.realAuth != '审核中'">
            审批认证
          </el-button>
          <el-button type="text" size="medium" v-if="isAuth(['ROOT', 'USER:UPDATE'])"
                     @click="showRepealModel(scope.row.id)" :disabled="scope.row.realAuth != '已认证'">
            撤销认证
          </el-button>
          <el-button type="text" size="medium" v-if="isAuth(['ROOT', 'USER:UPDATE'])"
                     @click="showUpdatePanel(scope.row.id)">设置状态
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    expand: function (row, expandedRows) {
      ...
      if (expandedRows.length > 0) {
        if (row) {
          ...
          // console.log(data);
          that.$http('driver/searchDriverRealSummary', 'POST', data, false, function (resp) {
            //console.log(resp);
